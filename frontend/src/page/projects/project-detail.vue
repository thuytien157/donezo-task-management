  ...
  <div>
    <div class="project-info-header shadow-sm " style="background-color: white;">
      ...
      <div class="project-actions">
        <div class="action-item"><i class="bi bi-bar-chart-line-fill"></i></div>
        <div class="action-item"><i class="bi bi-list"></i></div>

        <button v-if="!isLoading && (isLeader || project_name === 'Dự án Todo App')" class="action-item share-button"
      ...
      <div class="list-container" v-for="(taskList, status) in tasks" :key="status">
        <div class="trello-list">
          <div class="list-header">
            <h3>{{ status }}</h3>
            <div class="list-actions">
              <span class="card-count">{{ taskList ? taskList.length : 0 }}</span>
              <div class="form-add">
                ...
                <form class="p-2 border shadow-sm bg-white form-detail" style="width: 300px" v-if="showDropdown[status]"
                    ...
                    <multiselect v-model="newAssignees" :options="member" :multiple="true" :close-on-select="false"
                      ...
                      <template slot="tag" slot-scope="{ option, remove }">
                        <span class="multiselect__tag custom-tag">
                          <span class="custom-avatar-circle" :style="{
                  ...
                  <div class="d-flex justify-content-between">
                    <div class="d-flex gap-1">
                      <button class="btn btn-sm create-btn" type="submit">Thêm</button>
            ...
            <template #item="{ element }">
              <div class="trello-card">
                <router-link :to="`/projects/${projectId}/tasks/${element.id}`" class="text-decoration-none text-dark">
                  <div class="card-content fw-semibold">{{ element.title }}</div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form class="d-flex mb-3 gap-2" @submit.prevent="sendEmail">
            ...
            <button class="btn btn-primary share-btn1 w-25" type="submit" v-else> Chia sẻ</button>
          </form>

          <!-- <div class="share-link-section d-flex justify-content-between align-items-center p-2 mb-3">
            <div class="share-link-info">
              <i class="bi bi-link-45deg link-icon"></i>
              <div class="link-text">
                <p class="mb-0">Bất kỳ ai có liên kết đều có thể tham gia với tư cách thành viên</p>
                <a href="#" class="link-action">Sao chép liên kết</a>
                <span class="mx-1">·</span>
                <a href="#" class="link-action">Xóa liên kết</a>
              </div>
            </div>
          ...
          <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
              <button class="nav-link active" id="members-tab" data-bs-toggle="tab" data-bs-target="#members"
                ...
                Thành viên của bảng thông tin <span class="member-count">1</span>
              </button>
            </li>
              ...
              <div class="member-list-item d-flex justify-content-between align-items-center" v-for="value in member"
                ...
                <div class="d-flex align-items-center">
                  ...
                  <div class="member-info">
                    <p class="mb-0 member-name">{{ value.fullname }} <span class="text-muted">(bạn)</span></p>
  ...
  setup(props) {
    ...
    const checkUserRole = (members) => {
      ...
      if (userString) {
        ...
        const foundMember = members.find(m => m.id === currentUser.value.id);

        if (foundMember && foundMember.role === 'leader') {
          isLeader.value = true;
        } else {
    ...
    const onTaskChange = async (event, currentStatus) => {
      if (event.added) {
        ...
        if (oldStatus !== newStatus) {
          try {
            await axios.put('http://127.0.0.1:8000/api/task/update-status', {
    ...
    const sendEmail = async () => {
      ...
      try {
        await axios.post('http://127.0.0.1:8000/api/invite-member', {
