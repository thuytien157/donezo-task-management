<template>
  <div class="project-info-header" style="background-color: white">
  ...
  <div id="trello-board" v-if="!isListView">
        ...
        <div class="trello-list skeleton-list">
          ...
          <div class="card-container">
            <div class="trello-card skeleton-card" v-for="i in 3" :key="i">
              ...
              <div class="card-meta">
                <div class="skeleton-card-meta-avatar" v-for="j in 2" :key="j"></div>
              </div>
            </div>
          </div>
    ...
    <template v-else>
      <div class="list-container" v-for="(taskList, status) in tasks" :key="status">
        <div class="trello-list">
          <div class="list-header">
            ...
            <div class="list-actions">
              ...
              <div class="form-add">
                ...
                <form class="p-2 border shadow-sm bg-white form-detail" style="width: 300px" v-if="showDropdown[status]"
                  @submit.prevent="insertTask(projectId, status)">
                  <div>
                    <label class="form-label fw-normal" style="font-size: 14px">Tiêu đề nhiệm vụ</label><br />
                  ...
                  <div>
                    <label class="form-label fw-normal" style="font-size: 14px">Người được phân công</label><br />
                    <small class="text-danger" v-if="
                      errors && errors.user_id && errors.user_id.length > 0
                    ">
                      {{ errors.user_id[0] }}
                    </small>
                    <multiselect v-model="newAssignees" :options="member" :multiple="true" :close-on-select="false"
                      :clear-on-select="false" :preserve-search="true" placeholder="Tìm kiếm hoặc chọn người"
                      label="email" track-by="id" :preselect-first="false" :taggable="true" @tag="addTag">
                  ...
                  <div>
                    <label class="form-label fw-normal" style="font-size: 14px">Thời hạn hoàn thành</label><br />
                    <small class="text-danger" v-if="
                      errors && errors.deadline && errors.deadline.length > 0
                    ">
                      {{ errors.deadline[0] }}
                    </small>
                    <input type="text" :ref="(el) => (datetimePickerRefs[status] = el)" placeholder="Chọn ngày giờ"
                      class="form-control mb-3" v-model="deadline" />
                  </div>
                  <div class="d-flex justify-content-between">
                    <div class="d-flex gap-1">
                      ...
                      <router-link :to="`/projects/${projectId}/tasks/new`" class="btn btn-sm btn-secondary">
                        Chi tiết
                      </router-link>
                    </div>
          ...
          <draggable class="card-container" :list="taskList" :group="{ name: 'tasks', pull: true, put: true }"
            item-key="id" @change="onTaskChange($event, status)" @start="onDragStart($event, status)">
            <template #item="{ element }">
              <div class="trello-card">
                <router-link :to="`/projects/${projectId}/tasks/${element.id}`" class="text-decoration-none text-dark">
                  <div class="card-content fw-semibold">
                    {{ element.title }}
                  </div>
                  <i class="bi bi-clock text-dark" style="font-size: 14px; margin-right: 5px"></i>
                  <span>{{ formatDateTime(element.deadline) }}</span>
                  <div class="card-meta">
                    <img v-for="user in element.assignees" :key="user.id" :src="user.avatar"
                      style="width: 18px; border-radius: 2px; margin-left: 4px" alt="" />
                  </div>
                </router-link>
              </div>
            </template>
          ...
          <div class="form-add">
            <span class="add-card-placeholder" @click="toggleDropdown(status)">+ Thêm thẻ khác</span>
          </div>
        </div>
      </div>
    </template>
  ...
  <div id="list-view" v-else class="list-container-list-view">
    <div class="list-view-group" v-for="(taskList, status) in tasks" :key="status">
      ...
      <router-link :to="`/projects/${projectId}/tasks/${task.id}`" class="text-decoration-none text-dark"
        ...
        <div class="list-view-item">
          ...
          <div class="list-view-item-assignees">
            <img v-for="user in task.assignees" :key="user.id" :src="user.avatar" class="assignee-avatar" alt="" />
          </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <small class="text-danger" v-if="errors && errors.email && errors.email.length > 0">
            {{ errors.email[0] }}
          </small>

          <form class="d-flex mb-3 gap-2" @submit.prevent="sendEmail">
            <input type="email" class="form-control w-75" placeholder="Địa chỉ email" v-model="email" />

            <button disabled="" type="button" class="btn btn-primary share-btn w-25" v-if="sendLoading">
            ...
            <button class="btn btn-primary share-btn1 w-25" type="submit" v-else>
              Chia sẻ
            </button>
          </form>
          ...
          <div class="tab-content mt-3" id="myTabContent">
            <div class="tab-pane fade show active" id="members" role="tabpanel" aria-labelledby="members-tab">
              <div class="member-list-item d-flex justify-content-between align-items-center" v-for="value in member"
                :key="value.id">
                ...
                <button class="btn btn-light admin-role-btn" type="button" v-if="value.pivot.role == 'member'">
                  Thành viên
                </button>
                <button class="btn btn-light admin-role-btn" type="button" v-else>
                  Trưởng dự án
                </button>
              </div>
            </div>
...
export default {
  ...
  setup(props) {
    ...
    const checkUserRole = (members) => {
      const foundMember = members.find((m) => String(m.id) === String(userId.value));
    ...
    const getTaskByIdProject = async (id) => {
      try {
        ...
        console.log('g' + isGroup.value);
        console.log('l' + isLeader.value);
...
.dark-mode #trello-board {
  background-color: #1a1d26;
}

.dark-mode .trello-list {
...
.dark-mode .icon-add:hover,
.dark-mode .icon-more:hover {
...
.dark-mode .trello-card:hover {
  background-color: #555555;
}

.dark-mode .trello-card a {
  color: #f0f0f0 !important;
  /* Màu cho liên kết trong thẻ */
}

.dark-mode .trello-card i.bi-clock {
  color: #ccc !important;
}

.dark-mode .add-card-placeholder {
  color: #f0f0f0;
}

.dark-mode .add-card-placeholder:hover {
  background-color: #4a4a4a;
  color: #fff;
}

.dark-mode .form-detail {
  background-color: #5f5f5f !important;
  border-color: #555 !important;
  /* box-shadow: 0 0.5rem 1rem rgba(94, 93, 93, 0.9) !important; */
}
...
.dark-mode .member-info .member-role {
  color: #ecebeb;
}

.dark-mode .admin-role-btn {
  background-color: #3a3a3a;
  color: #f0f0f0;
  border-color: #555;
}

.dark-mode .admin-role-btn:hover {
  background-color: #4a4a4a;
}

/* List View */
.dark-mode .list-view-group-title {
  background-color: #2a2a2a;
  color: #f0f0f0;
}

.dark-mode .list-view-item-header {
  background-color: #3a3a3a;
  color: #f0f0f0;
  border-bottom-color: #555;
}

.dark-mode .list-view-item {
  background-color: #3a3a3a;
  border-bottom-color: #555;
  color: #f0f0f0;
}

.dark-mode .list-view-item:hover {
  background-color: #4a4a4a;
}
...
.dark-mode .list-view-item-deadline {
  color: #ccc;
}
...
.dark-mode .skeleton-list {
  background-color: #333 !important;
}

.dark-mode .skeleton-title,
.dark-mode .skeleton-card-count,
.dark-mode .skeleton-card,
.dark-mode .skeleton-card-content,
.dark-mode .skeleton-card-deadline,
.dark-mode .skeleton-card-meta-avatar,
.dark-mode .skeleton-add-card-placeholder {
  background-color: #444;
